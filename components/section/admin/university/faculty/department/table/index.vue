    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          ...
          ></v-text-field>
          <v-spacer></v-spacer>
          <v-btn color="primary" dark class="mb-2" @click="showDialog"> Departemen Baru </v-btn>
                    ...
                    <v-row dense>
                      ...
                      <v-col cols="12" sm="6">
                        <div class="mb-1 text-truncate">
                          <label for="input-faculty-id" class="text-body2 sblack60--text"> Fakultas </label>
                        </div>
                        <v-autocomplete
                          id="input-faculty-id"
                          v-model="editedItem.facultyId"
          ...
          <v-dialog v-model="dialogDelete" max-width="600px">
            <v-card>
              <v-card-title class="">Apakah anda yakin untuk menghapus Departemen ini?</v-card-title>
              <MessageInfo
                :messages.sync="errorMessage"
                class="mb-4"
                card-class="sred20"
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Fakultas', value: 'faculty.name' },
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? 'Tambah Departemen' : 'Ubah Departemen'
    }
  },
  ...
  methods: {
    ...
    deleteItemConfirm() {
      this.deleteDepartment(this.editedItem)
    },
    ...
    save() {
      this.postDepartment(this.editedItem)
    },

    async postDepartment(form) {
      if (this.$refs.form.validate()) {
        ...
        try {
          const a =
            this.editedIndex > -1
              ? await this.$repo.university.updateDepartment(form.id, form)
              : await this.$repo.university.createDepartment(form)
          const res = a.data
          if (res && res.status) {
    ...
    async deleteDepartment(form) {
      this.errorMessage = {}
      this.loading = true
      try {
        const a = await this.$repo.university.deleteDepartment(form.id, form)
        const res = a.data
        if (res && res.status) {
          this.$YAlert.show({ content: res.messages, timeout: '2000' })
          this.initialize()
